<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Edit Worker</h2>
                <p class="text-gray-500 mt-1">{{ $worker->name }}</p>
            </div>
            ...
            <div class="card bg-white shadow-xl">
                <div class="card-body">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-gray-900">Edit Worker Details</h3>
                            <p class="text-sm text-gray-500">Update the worker information below</p>
                        </div>
                    </div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="form-control">
                                ...
                                <textarea name="address" rows="2"
                                    ...
                                    class="textarea textarea-bordered">{{ old('address', $worker->address) }}</textarea>
                                <label class="label">
                                    ...
                                    <div class="relative">
                                        <select name="labor_type" class="select select-bordered w-full appearance-none" required>
                                            ...
                                            @foreach($laborTypes as $key => $label)
                                                <option value="{{ $key }}" {{ old('labor_type', $worker->labor_type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="relative">
                                        <select name="category" class="select select-bordered w-full appearance-none">
                                            ...
                                            @foreach($categories as $key => $label)
                                                <option value="{{ $key }}" {{ old('category', $worker->category) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                            @endforeach
                                        </select>
                                        <div class="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
                                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="form-control">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="daily_wage" value="{{ old('daily_wage', $worker->daily_wage) }}" step="0.01"
                                        placeholder="0.00"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="form-control">
                                    ...
                                    <div class="card bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200">
                                        <div class="card-body p-4">
                                            <label class="label cursor-pointer p-0 flex items-center">
                                                <input type="checkbox" name="is_active" class="checkbox checkbox-primary" {{ old('is_active', $worker->is_active) ? 'checked' : '' }} value="1">
                                                <span class="label-text ml-2 font-semibold text-gray-900">Active Worker</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-control">
                                    <label class="label flex items-center">
                                        <span class="label-text font-semibold text-gray-900">Terminate Date</span>
                                        <span class="label-text-alt text-gray-400">(Optional)</span>
                                    </label>
                                    <div class="relative">
                                        <input type="date" name="terminate_date" value="{{ old('terminate_date', $worker->terminate_date?->format('Y-m-d')) }}"
                                            class="input input-bordered w-full" />
                                        <div class="absolute right-3 top-1/2 -translate-y-1/2">
                                            <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="relative">
                                    <select name="primary_contractor_id" class="select select-bordered w-full appearance-none">
                                        ...
                                        @foreach($contractors as $contractor)
                                            <option value="{{ $contractor->id }}" {{ old('primary_contractor_id', $worker->primary_contractor_id) == $contractor->id ? 'selected' : '' }}>
                            ...
                            <div class="form-control">
                                ...
                                <div class="relative">
                                    <input type="date" name="hire_date" value="{{ old('hire_date', $worker->hire_date?->format('Y-m-d')) }}"
                                        class="input input-bordered w-full" />
                                    <div class="absolute right-3 top-1/2 -translate-y-1/2">
                                        <svg class="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                            ...
                            <div class="form-control">
                                ...
                                <textarea name="notes" rows="3"
                                    ...
                                    class="textarea textarea-bordered">{{ old('notes', $worker->notes) }}</textarea>
                                <label class="label">
                                    <span class="label-text-alt">Add any additional information about the worker</span>
                                </label>
                            </div>

                            <!-- Worker Stats Info Card -->
                            <div class="card bg-gradient-to-r from-cyan-50 to-blue-50 border border-cyan-200">
                                <div class="card-body p-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="flex-1">
                                            <h4 class="font-bold text-gray-900">Worker Statistics</h4>
                                            <div class="grid grid-cols-2 gap-x-4 gap-y-1 mt-2 text-sm">
                                                <div class="flex items-center gap-2">
                                                    <span class="text-gray-600">Days Worked:</span>
                                                    <span class="font-bold">{{ $worker->total_days_worked }}</span>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <span class="text-gray-600">Total Wages:</span>
                                                    <span class="font-bold text-green-600">{{ number_format($worker->total_wages_earned, 2) }}</span>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <span class="text-gray-600">Payments:</span>
                                                    <span class="font-bold text-blue-600">{{ number_format($worker->total_payments_received, 2) }}</span>
                                                </div>
                                                <div class="flex items-center gap-2">
                                                    <span class="text-gray-600">Balance:</span>
                                                    <span class="font-bold {{ $worker->total_wages_earned - $worker->total_payments_received > 0 ? 'text-orange-600' : 'text-green-600' }}">
                                                        {{ number_format($worker->total_wages_earned - $worker->total_payments_received, 2) }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
